...
function Three_Member_Team() {
  const [teamName,setTeamName]=useState('');
  const [p1,setP1]=useState('');
  const [p2,setP2]=useState('');
  const [p3,setP3]=useState('');
  const [{schoolId,activeEvent},dispatch]=useStateValue();

  function handleEvent(e){
    e.preventDefault();
    {if(teamName,p1,p2,p3)
        {
    axios.post('/vinterbash/add_team',{teamName,p1,p2,p3,activeEvent,schoolId
        // sellerEmail,
        })
    .then((data)=>{
      setTeamName('')
  ...
  return (
    <div className='login'>
        ...
        {/* PARTICIPANT 3 */}
        <h5>Participant 3</h5>
        <input type='text' value={p3} placeholder="Type Candidate's Name" onChange={(e)=>{setP3(e.target.value);}} className='register_form'/>

...
router.post('/registeredEvents', async (req, res) => {
    try {
        ...
        rows.forEach(row => {
            ...
            teacher1Phone = row.teacher1number;
            teacher2Phone = row.teacher2number;
        });

        res.status(200).json({ schoolId, schoolName,teacher1Phone,teacher2Phone, teacher1Name, teacher2Name, fullyRegistered: fully, partiallyRegistered: partially, notRegistered: none });
    } catch (error) { res.status(500).json({ error: "Internal Server Error" }); }
});
